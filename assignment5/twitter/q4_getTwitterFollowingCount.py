...
if __name__ == "__main__":
    if not OAUTH_TOKEN:
		...
		print '%-15s %-20s %-25s' %('Friends_count','Friends-screen-name','Friends-name')	
		print '----------------------------------------------------------------------------------------'		
		...
		while True:
			...
			for init_url in raw_res:
				counter = counter + 1
				print '%-15d %-20s %-25s' %(init_url['friends_count'],init_url['screen_name'].encode('ascii', 'replace'),init_url['name'].encode('ascii', 'replace'))
				freqList.append(int(init_url['friends_count']))								
